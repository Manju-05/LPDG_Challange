# Comprehensive Project Execution Guide & Technical Reference (GUIDE.md)

This guide provides a complete, step-by-step walkthrough for running, testing, demonstrating, and evaluating the **LPDG Innovation Hub Selection Challenge 2026** codebase (**Part 1: Deterministic Prioritization Pipeline** and **Part 2: Track B — Software Development REST Web Service**).
...
```
- **What it does**: Runs the official challenge validator script to check schema conformity, column names, row counts, week date sequences, rank bounds (1–15), gateway ID formats, and character length limits on reason text ($\le 300$ chars).
...
- **Test Breakdown (29 Tests Total)**:
  1. [`tests/test_temporal_cutoff.py`](tests/test_temporal_cutoff.py) (2 tests): Asserts strictly 0 data leakage past Monday 00:00 UTC and validates withholding of the mid-February engineer review sheet.
  2. [`tests/test_schema_and_validation.py`](tests/test_schema_and_validation.py) (2 tests): Tests synthetic fixtures and official validator compliance.
  3. [`tests/test_reproducibility.py`](tests/test_reproducibility.py) (1 test): Verifies bit-for-bit deterministic reproducibility across back-to-back runs.
  4. [`tests/test_components.py`](tests/test_components.py) (3 tests): Tests ID normalizer, reason length limits, and multi-week cooldown decay.
  5. [`tests/test_ranker_interface.py`](tests/test_ranker_interface.py) (3 tests): Validates polymorphic `BaseRanker` interface and dynamic ranker swapping.
  6. [`tests/test_api.py`](tests/test_api.py) (18 tests): Comprehensive REST API test suite covering liveness probes, fleet health summaries, rankings queries, dynamic latest partition discovery, date validation, single-gateway diagnostics, 8-week history trends, concurrent run locking (`409 Conflict`), dynamic disk reload without restart, and default canonical `POST /run` generation.
- **Expected Output**: `============================= 29 passed in ...s =============================`
...
| **`GET /fleet/summary`** | `?week=YYYY-MM-DD` | Macro operational health overview across the entire fleet for a given week (active gateways, 3$\sigma$ breach count, silent units count, and average meter fail rate). | `curl "http://127.0.0.1:8000/fleet/summary?week=2026-02-02"` |
| **`GET /rankings`** | `?week=YYYY-MM-DD`<br>`?week=latest`<br>`?ranker=composite\|baseline` | Returns top 15 prioritized gateways for field dispatch with scores and operations reasons. Dynamically scans disk if `?week=latest` is supplied. | `curl "http://127.0.0.1:8000/rankings?week=2026-02-02"`<br>`curl "http://127.0.0.1:8000/rankings?week=latest"` |
| **`GET /gateways/{id}`** | `gateway_id` in path<br>`?week=YYYY-MM-DD`<br>`?ranker=composite\|baseline` | Diagnostic details for a single gateway. Normalizes both bare MAC (`0A2778A31BE3`) and colon MAC (`0a:27:78:a3:1b:e3`). Returns 404 if not found. | `curl "http://127.0.0.1:8000/gateways/0A2778A31BE3?week=2026-02-02"` |
| **`GET /gateways/{id}/history`** | `gateway_id` in path<br>`?ranker=composite\|baseline` | Chronological 8-week trend for a gateway showing weekly ranks, scores, offline hours, and whether a technician visit was recommended. | `curl "http://127.0.0.1:8000/gateways/0A2778A31BE3/history"` |
| **`POST /run`** | `?ranker=composite\|baseline`<br>`?out=predictions.csv` | Re-executes the prioritization pipeline dynamically over mounted disk data without server restart. Features a non-blocking mutex lock (`409 Conflict` on concurrent runs). | `curl -X POST "http://127.0.0.1:8000/run"` |
...
- **Implementation**: In [`src/features.py`](src/features.py), all telemetry is strictly filtered to $[T - 28\text{d}, T)$, meter reads to $\text{week\_date} < T$, field visits to $\text{visited\_on} < T$, and engineer review to $T \ge \text{2026-02-16}$.
...
- **Implementation**: Filters `gateway_master.csv` (332 entries) to active commissioned hardware: $\text{installed\_on} \le T \;\text{AND}\; (\text{decommissioned\_on} \ge T \;\lor\; \text{NULL})$.
...
### 3. Multi-Week Episode Cooldown ($0.10\times \to 0.25\times \to 0.50\times \to 1.00\times$)
- **Implementation**: In [`src/ranker.py`](src/ranker.py), visited gateways receive a progressive decay based on weeks since last visit.
...
### 4. Exponential Meter Read Staleness Decay ($0.8^{\text{weeks\_lag}}$)
- **Implementation**: Multiplies meter read failure rate by $0.8^{\text{weeks\_lag}}$.
- **Why**: `meter_read_success.csv` ends on 2026-01-26. In March 2026 (week 8), the report is 8 weeks old. The decay ensures the system prioritizes fresh March telemetry over aging historical meter reports.
...
## 6. Final Repository Verification Summary

- **Branches on GitHub**:
  - `main`: Complete Part 1 (pipeline + docs + predictions) + Part 2 (Track B REST API service + 29 tests).
  - `part-2-software-development`: Synchronized Track B development branch.
- **Tracked Files**: 100% clean (no prompt notes, no raw datasets, no caches).
- **Submission File**: `predictions.csv` (120 rows, 8 weeks $\times$ 15 visits, validated).
